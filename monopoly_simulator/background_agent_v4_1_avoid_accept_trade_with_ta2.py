...
if player is TA2 (player_1) -> reject the offer!
    ...
    if (player.outstanding_trade_offer['cash_offered'] <= 0 and len(player.outstanding_trade_offer['property_set_offered'])==0) \
        ...
        if reject_flag == 0:
            ...
            # groups are only requested from you in the trade.)
                ...
                # anti umass hacker
                # if player is TA2 (player_1) -> reject the offer!
                if player.outstanding_trade_offer['from_player'].player_name == 'player_1':
                    logger.debug(player.player_name + " rejected trade offer from " + player.outstanding_trade_offer['from_player'].player_name + "because of umass hacker")
                    reject_flag = 1
                elif (player.outstanding_trade_offer['cash_wanted'] - player.outstanding_trade_offer['cash_offered']) >= player.current_cash:
                # ------------------------------------
