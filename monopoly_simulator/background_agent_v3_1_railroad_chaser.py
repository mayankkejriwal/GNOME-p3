...
respective functions must adhere in their signatures to the examples here. The agent in this file is simple and rule-based,
 ...
 Main goal: This agent want to own railroad locations as its best. 
 modified functions: 
    1. auction (function make_bid): make more bid = current_bid+(player.current_cash-current_bid)/4 (same amount with the condition will_property_complete_set but bid on railroad)
    2. buy property (function make_buy_property_decision): if asset is railroad , and still have positive cash balance afterwards (though it may be less than go_increment)
    3. trade: never offer owned railroad to others, only make offer for railroad from others!       
        a. accept_sell_property_offer:
            accept condition: if player.current_cash - player.outstanding_property_offer['price'] >= current_gameboard['go_increment']/2
        b. make_trade_offer
            (1) agent_helper_functions_v3.identify_property_trade_offer_to_player_railroad_chaser: never offer railroad to others
            (2) agent_helper_functions_v3.identify_property_trade_wanted_from_player_railroad_chaser: only looking for railroad, 
                and the cash_offered rate will be [0.75, 1.5, 3, 6], in function curate_trade_offer_multiple_players_railroad_chaser, there 
                is addtional multiplying factor 2. In other words, this player will offer [300, 600, 1200, 2400] depending on how many railroad this
                player owned. For exmample, if player have 0 railroad, it will offer 300 for a railroad. if player have 3 railroad, it will offer 2400 
                for the last railroad.
            (3) agent_helper_functions_v3.curate_trade_offer_multiple_players_railroad_chaser.
                change the rate for all request to * 1.5 (because we only request for railroad)
                decrease the rate for for cash_wanted / 1.875 if we are using asset to trade railroad with others (to make the success probability increase)
...
def make_out_of_turn_move(player, current_gameboard, allowable_moves, code):
    ...
    if "accept_sell_property_offer" in allowable_moves:
        ...
        logger.debug('('+player.player_name+' currently has cash balance of '+str(player.current_cash)+')')
        ### railroad_chaser only: accept_sell_property_offer
        if player.outstanding_property_offer['asset'].loc_class == 'railroad' and player.current_cash - player.outstanding_property_offer['price'] >= current_gameboard['go_increment']/2:
            logger.debug(player.player_name + ': I accepted offer from ' + player.outstanding_property_offer['from_player'].player_name + 'with the location ' + player.outstanding_property_offer['asset'].name + ' because I am railroad_chaser.')
    ...
    if player.status != 'current_move': # these actions are considered only if it's NOT our turn to roll the dice.
        ...
        if player.current_cash < current_gameboard['go_increment'] and "make_trade_offer" in allowable_moves:
            ...
            potential_offer_list = agent_helper_functions.identify_property_trade_offer_to_player_railroad_chaser(player, current_gameboard)
            potential_request_list = agent_helper_functions.identify_property_trade_wanted_from_player_railroad_chaser(player, current_gameboard)
            param_list = agent_helper_functions.curate_trade_offer_multiple_players_railroad_chaser(player, potential_offer_list, potential_request_list, current_gameboard, purpose_flag=1)
            #logger.debug(param)
            return_action_list = []
            return_param_list = []
            ...
            potential_offer_list = agent_helper_functions.identify_property_trade_offer_to_player_railroad_chaser(player, current_gameboard)
            potential_request_list = agent_helper_functions.identify_property_trade_wanted_from_player_railroad_chaser(player, current_gameboard)
            param_list = agent_helper_functions.curate_trade_offer_multiple_players_railroad_chaser(player, potential_offer_list, potential_request_list, current_gameboard, purpose_flag=2)
            #logger.debug(param)
            return_action_list = []
            return_param_list = []
            ...
            if param_list and player.agent._agent_memory['previous_action'] != "make_trade_offer":  # we only make one offer per turn. Otherwise we'd
                ...
                for param in param_list:
                    ...
                    param['offer']['property_set_wanted'] = prop_set_wanted
                    param['current_gameboard'] = 'current_gameboard'
                    logger.debug(player.player_name + ': I currently owed number_of_railroad = ' + str(player.num_railroads_possessed))
                    logger.debug(player.player_name + ': I am making an offer to trade ' +
                                 ', '.join([str(pps) for pps in param['offer']['property_set_offered']])+ ' (with cash offered ' +
                                 str(param['offer']['cash_offered']) + ') to ' +
                                 param['to_player'] + ' for '+ ', '.join([str(pps) for pps in param['offer']['property_set_wanted']]) + ' (with cash wanted ' +
                                 str(param['offer']['cash_wanted']) + ') because I am a railroad_chaser')
                    player.agent._agent_memory['previous_action'] = "make_trade_offer"
                    return_action_list.append("make_trade_offer")
                    return_param_list.append(param)
...
def make_buy_property_decision(player, current_gameboard, asset):
    ...
    ### railroad_chaser only:
    (0) if asset is railroad , and still have positive cash balance afterwards (though it may be less than go_increment).
    ###
    ...
    ###
    ### railroad_chaser only:
    if asset.loc_class == 'railroad' and asset.price <= player.current_cash:
        logger.debug(player.player_name + ': I will attempt to buy ' + asset.name + ' from the bank because I am railroad_chaser.')
        decision = True
    ###
...
def make_bid(player, current_gameboard, asset, current_bid):
    ...
    ###
    ### railroad_chaser only:
    if asset.loc_class == 'railroad' and current_bid < player.current_cash:
        logger.debug(player.player_name + ': I will to bid more on ' + asset.name + ' because I am railroad_chaser.')
