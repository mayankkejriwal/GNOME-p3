...
def make_out_of_turn_move(player, current_gameboard, allowable_moves, code):
    ...
    if "accept_arbitrary_interaction" in allowable_moves:

        # from_player, to_player, interaction_id, decision, current_gameboard
        for inter_id, item in player.interaction_dict.items():
            if item['from_player'].agent._agent_memory['free_rent'][inter_id]['location'].owned_by == item['from_player'] and \
                item['from_player'].agent._agent_memory['free_rent'][inter_id]['to_location'].owned_by == player:
                from_location_rent = RealEstateLocation.calculate_rent(item['from_player'].agent._agent_memory['free_rent'][inter_id]['location'], current_gameboard)
                to_location_rent = RealEstateLocation.calculate_rent(item['from_player'].agent._agent_memory['free_rent'][inter_id]['to_location'], current_gameboard)
                print(f"from_rent: {from_location_rent}, to_rent: {to_location_rent}")
                if from_location_rent > to_location_rent:
                    param = dict()
                    param['from_player'] = item['from_player']
                    param['to_player'] = player.player_name
                    param['interaction_id'] = inter_id
                    param['decision'] = True
                    param['current_gameboard'] = 'current_gameboard'

                    return 'accept_arbitrary_interaction', param
                else:
                    param = dict()
                    param['from_player'] = item['from_player']
                    param['to_player'] = player.player_name
                    param['interaction_id'] = inter_id
                    param['decision'] = False
                    param['current_gameboard'] = 'current_gameboard'

                    return 'accept_arbitrary_interaction', param

        # for inter_id, item in player.interaction_dict.items():  # accept all offer without reasons
        #     param = dict()
        #     param['from_player'] = item['from_player']
        #     param['to_player'] = player.player_name
        #     param['interaction_id'] = inter_id
