...
SNEAKER_TRADER_CASH_OFFERED = 0.1
...
def identify_improvement_opportunity_limit(player, current_gameboard, num=3):
    ...
    for c in player.full_color_sets_possessed:
        ...
        for asset in c_assets:
            if can_asset_be_improved(asset, c_assets, current_gameboard) and asset.price_per_house <= player.current_cash:
...
def identify_improvement_opportunity_reverse_improvement(player, current_gameboard):
    """
    @ author: Min-Hsueh Chiu
    @ date: sep. 28, 2021
    For relation novelty: reverse_improvements
    Having same logic with function identify_improvement_opportunity except the hotel will be improved first, and then house
    :param player:
    :param current_gameboard:
    :return: a parameter dictionary or None. The parameter dictionary, if returned, can be directly sent into
    action_choices.improve_property by the calling function.
    ...
    for c in player.full_color_sets_possessed:
        c_assets = current_gameboard['color_assets'][c]
        for asset in c_assets:
            if can_asset_be_improved_reverse_improvement(asset,c_assets) and asset.price_per_house<=player.current_cash: # player must be able to afford the improvement
                potentials.append((asset,asset_incremental_improvement_rent_reverse_improvement(asset)-asset.price_per_house))
    if potentials:
        sorted_potentials = sorted(potentials, key=lambda x: (x[1], x[0].name), reverse=True) # sort in descending order
        param = dict()
        param['player'] = player
        ...
        param['add_hotel'] = True
        param['add_house'] = False
        if param ['asset'].num_hotels == current_gameboard['bank'].hotel_limit:
            param['add_house'] = True
            param['add_hotel'] = False
        return param
    else:
...
def can_asset_be_improved_reverse_improvement(asset, same_color_assets):
    """
    # @Author  : Min-Hsueh Chiu
    # @Time    : Sep. 28, 2021
    Using in function identify_improvement_opportunity_reverse_improvement.
    :param asset: asset that needs to be improved
    :param same_color_assets: other assets of the same color
    :return: True if asset can be improved else False
    """
    ...
    for c_asset in same_color_assets:
        ...
        if c_asset == asset:
            continue
        if c_asset.num_hotels == 1 and asset.num_houses == 0 :
            return True # we can build a house on asset
        if c_asset.num_houses > asset.num_houses:
            return True
        if c_asset.num_houses == asset.num_houses:
            count += 1
...
def asset_incremental_improvement_rent_reverse_improvement(asset):
    """
    # @Author  : Min-Hsueh Chiu
    # @Time    : Sep. 28, 2021
    Using in function identify_improvement_opportunity_reverse_improvement.
    ...
    if asset.num_houses == 4:
        logger.error("Exception") # there is no incremental improvement possible. how did we get here?
    if asset.num_houses == 3:
        return asset.rent_4_houses - asset.rent_3_houses
    elif asset.num_houses == 2:
        return asset.rent_3_houses - asset.rent_2_houses
    elif asset.num_houses == 1:
        return asset.rent_2_houses - asset.rent_1_house
    elif asset.num_houses == 0 and asset.num_hotels == 1:
        return asset.rent_1_house - asset.rent_hotel
    else:
        return asset.rent_hotel - (asset.rent * 2) # remember, if the house can be improved, then it is monopolized, so twice the rent is being charged even without houses.
...
def identify_property_trade_offer_to_player_railroad_chaser(player, current_gameboard):
    """
    Base on identify_property_trade_offer_to_player, but it never give railroad to other, even if it is near bankruptcy
        ...
        if is_property_lone(player, a):
            for p in current_gameboard['players']:
                if p == player or p.status == 'lost':
                    ...
                    param['to_player'] = p
                    param['price'] = a.price*1.5 # 50% markup on market price.
                    if param['price'] > param['to_player'].current_cash:
                        # no point offering this to the player; they don't have money.
                        continue
...
def identify_property_trade_wanted_from_player_railroad_chaser(player, current_gameboard):
    """
    it only looking for the railroad properties.
    :param player: player who wants properties from other players and hence invokes this function
    :param current_gameboard: The gameboard data structure
    :return: a parameter dictionary or None. The parameter dictionary, if returned, can be directly sent into
    action_choices.make_sell_property_offer by the calling function.
    ...
    for p in current_gameboard['players']:
        if p == player or p.status == 'lost':
            ...
            for a in sorted_player_assets:
                ### railroad_chaser
                if a.loc_class == 'railroad':
                    railroad_purchase_rate = [0.75, 1.5, 3, 6]

                    param = dict()
                    param['from_player'] = p
                    param['asset'] = a
                    param['to_player'] = player
                    param['price'] = a.price * railroad_purchase_rate[player.num_railroads_possessed]
                    if param['price'] > player.current_cash:
                        # I don't have money to buy this property even at a reduced price.
                        continue
                    potentials.append((param, param['price']))
                ...
                # I dont need other property except railroad
...
def curate_trade_offer_multiple_players_railroad_chaser(player, potential_offer_list, potential_request_list, current_gameboard, purpose_flag):
    """
    Generates trade offers from the potential offer list, potential request list and the purpose indicated by
    the purpose flag for background_agent_v3.
    Allows a player to make trade offers to MULTIPLE PLAYERS simultaneously.
    ...
    if purpose_flag == 1:
        ...
        if not potential_offer_list and not potential_request_list:
            ...
            #constraint = request only 1 property (only request railroad)
            #nothing to offer (I have no lone properties that could lead other players into getting a monopoly) but I want
            #railroad, then I will request the property at 1.35 times the market price
            #provided that offer doesnot make me broke (Note that if I had properties to offer to the other player to help him or her get a monopoly
            #then I would only pay 0.75 times the market price. But now its my need, so I am willing to pay 1.25 times the market price.)
            player_list = []
            offer_list_multiple_players = []
            ...
            for item in potential_request_list:
                if item[0]['from_player'] not in player_list:
                    ...
                    trade_offer['cash_offered'] = 0
                    trade_offer['cash_wanted'] = 0
                    cash_to_be_offered_during_trade += item[0]['price'] * 2
                    if player.current_cash - cash_to_be_offered_during_trade > current_gameboard['go_increment']/2:
                        trade_offer['cash_offered'] = item[0]['price'] * 2
                        trade_offer['property_set_wanted'].add(item[0]['asset'])
                        param['from_player'] = player
                        param['to_player'] = item[0]['from_player']
                        param['offer'] = trade_offer
                ...
                if player_2 not in saw_players:
                    ...
                    for off in potential_offer_list:
                        if off[0]['to_player'] == player_2:
                            ...
                            found_player_flag = 1
                            trade_offer['property_set_offered'].add(off[0]['asset'])
                            trade_offer['cash_wanted'] = off[0]['price'] / 1.875
                            trade_offer['property_set_wanted'].add(req[0]['asset'])
                            trade_offer['cash_offered'] = req[0]['price'] * 2
                            param['from_player'] = player
                            param['to_player'] = player_2
                            param['offer'] = trade_offer
                            offer_list_multiple_players.append(param)
            ...
            if found_player_flag == 0:
                ...
                for item in potential_request_list:
                    if item[0]['from_player'] not in player_list:
                        ...
                        cash_to_be_offered_during_trade += item[0]['price']*2
                        if player.current_cash - cash_to_be_offered_during_trade > current_gameboard['go_increment']/2:
                            trade_offer['cash_offered'] = item[0]['price']*2  #(0.75*2=1.5)
                            trade_offer['property_set_wanted'].add(item[0]['asset'])
                            param['from_player'] = player
                            param['to_player'] = item[0]['from_player']
                            param['offer'] = trade_offer
...
def identify_improvement_opportunity_cash_conservator(player, current_gameboard):
    """
    can only afford asset.price_per_house <= player.current_cash * 0.1
    ...
    for c in player.full_color_sets_possessed:
        c_assets = current_gameboard['color_assets'][c]
        for asset in c_assets:
            if can_asset_be_improved(asset, c_assets, current_gameboard) and asset.price_per_house <= player.current_cash * 0.1: # player must be able to afford the improvement, but only 10% of cash
                potentials.append((asset, asset_incremental_improvement_rent(asset, current_gameboard) - asset.price_per_house))
    if potentials:
        sorted_potentials = sorted(potentials, key=lambda x: (x[1], x[0].name), reverse=True) # sort in descending order
        param = dict()
...
def identify_property_trade_wanted_from_player_sneaker_trader(player, current_gameboard):
    """
    Identify an opportunity to buy a property currently owned another player by making a
    trade offer for the purpose of increasing the number of monopolies. This is a 'strategic' function; there
    are many other ways/strategies to identify such sales than the one we use here. All we do is identify if
    ...
    if not potentials:
        ...
        random.seed(current_gameboard['seed'])
        random.shuffle(potentials)
        return potentials
        #sorted_potentials = sorted(potentials, key=lambda x: x[1], reverse=False)  # sort in ascending order
        #return sorted_potentials

def curate_trade_offer_multiple_players_sneaker_trader(player, potential_offer_list, potential_request_list, current_gameboard, purpose_flag):
    """
    this is used by the background_agent_v3_1_sneaky_trader, the difference is in cash_offered = 0.1
        ...
        if not potential_offer_list and not potential_request_list:
            ...
            for item in potential_request_list:
                if item[0]['from_player'] not in player_list:
                    ...
                    trade_offer['cash_offered'] = 0
                    trade_offer['cash_wanted'] = 0
                    cash_to_be_offered_during_trade += SNEAKER_TRADER_CASH_OFFERED
                    if player.current_cash - cash_to_be_offered_during_trade > current_gameboard['go_increment']/2:
                        #trade_offer['cash_offered'] = item[0]['price'] * 2
                        trade_offer['cash_offered'] = SNEAKER_TRADER_CASH_OFFERED
                        trade_offer['property_set_wanted'].add(item[0]['asset'])
                        param['from_player'] = player
                        param['to_player'] = item[0]['from_player']
                        param['offer'] = trade_offer
