...
Agent's detail:
 ...
 Main goal: This agent is sneaky trader who make the trade that:
            1. offer small amount money ($ 0.1) to trade other's property
 modified functions: 
    trade:
        (1) agent_helper_functions_v3.identify_property_trade_wanted_from_player_sneaker_trader:
        it will try all locations to trade with everyone else with rediculus price
            Not confining itself to only trade on: (original setting in background_agent_v3_1)
                a. real_estate
                b. Not full_color_sets_possessed
                c. is_property_lone and will_property_complete_set
        Also, it will not return the sorted potentials, instead, return random.shuffle(potentials) to avoid two sneaky_trader always trading same object,
        with seed random.seed(current_gameboard['seed'])

        (2) agent_helper_functions_v3.curate_trade_offer_multiple_players_sneaker_trader
            1. purpose_flag = 1, near bankruptcy, not change
            2. purpose_flag = 2
                ignore potential_offer_list, just use $ 0.1 to trade other's asset in potential_request_list
...
def make_out_of_turn_move(player, current_gameboard, allowable_moves, code):
    ...
    if player.current_cash < current_gameboard['go_increment'] and "make_trade_offer" in allowable_moves:
        ...
        potential_request_list = agent_helper_functions.identify_property_trade_wanted_from_player_sneaker_trader(player, current_gameboard)
        param_list = agent_helper_functions.curate_trade_offer_multiple_players_sneaker_trader(player, potential_offer_list, potential_request_list, current_gameboard, purpose_flag=1)
        #logger.debug(param)
        return_action_list = []
        return_param_list = []
        ...
        if param_list and player.agent._agent_memory['previous_action'] != "make_trade_offer": # we only make one offer per turn. Otherwise we'd
            ...
            for param in param_list:
                ...
                logger.debug(player.player_name + ': I am making an offer to trade ' +
                             ', '.join([str(pps) for pps in
                                        param['offer']['property_set_offered']]) + ' (with cash offered ' +
                             str(param['offer']['cash_offered']) + ') to ' +
                             param['to_player'] + ' for ' + ', '.join(
                    [str(pps) for pps in param['offer']['property_set_wanted']]) + ' (with cash wanted ' +
                             str(param['offer']['cash_wanted']) + ') because I am a sneaker_trader')
        ...
        potential_offer_list = agent_helper_functions.identify_property_trade_offer_to_player(player, current_gameboard)
        potential_request_list = agent_helper_functions.identify_property_trade_wanted_from_player_sneaker_trader(player, current_gameboard)
        param_list = agent_helper_functions.curate_trade_offer_multiple_players_sneaker_trader(player, potential_offer_list, potential_request_list, current_gameboard, purpose_flag=2)
        #logger.debug(param)
        return_action_list = []
        return_param_list = []
